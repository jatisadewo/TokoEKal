...
class Cart extends MY_Controller 
{
    ...
    public function add()
    {
        $inp = (object) $this->input->post(null, true);
        $available = $this->ModelCetak->available(['id' => $inp->id_product])->row();
        ...
        if (!$_POST || $this->input->post('quantity') < 1) {
            ...
            $inp = (object) $this->input->post(null, true);
            $slug = $this->ModelCetak->available(['id' => $inp->id_product])->row();
            $this->session->set_flashdata('error', 'Oops! Stock Tidak Tersedia.');
            redirect(base_url("shop/detail/$slug->slug"));
        }
